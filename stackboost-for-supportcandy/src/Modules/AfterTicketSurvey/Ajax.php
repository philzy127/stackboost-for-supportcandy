...
class Ajax {
    ...
    public function save_question() {
        ...
        if ( $question_id ) {
            ...
            $result = $wpdb->update( $this->questions_table_name, $data, [ 'id' => $question_id ] ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            if ( false === $result ) {
            ...
            // phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
            $result = $wpdb->insert( $this->questions_table_name, $data ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            if ( false === $result ) {
