...
*   **Lines 314, 402**
    ...
    *   **Context**: Accessing `$_POST` inside a loop or variable assignment for meta box saving.
    *   **Reason**: The linter flags the initial access to `$_POST` variables even when they are immediately passed to sanitization functions. The context is safe because sanitization happens on the very same line.
...
*   **Lines 714, 771**
    ...
    *   **Context**: Outputting the content of the "Contact Widget" (`$widget_content`).
    *   **Reason**: The `$widget_content` variable is a string of HTML constructed within the method. It contains trusted HTML (e.g., links generated by `get_formatted_phone_numbers_html` which returns sanitised anchor tags) and other values that are explicitly escaped (`esc_html`) at the point of concatenation.
...
*   **Lines 290**
    ...
    *   **Context**: Accessing `$_FILES['import_file']`.
    *   **Reason**: The line accesses the array structure to retrieve the file information. Specific properties like `name` are sanitized via `sanitize_file_name` in subsequent steps, ensuring safe usage.
...
*   **Lines 109, 114**
    *   **Rule**: `WordPress.DB.DirectDatabaseQuery.DirectQuery`, `WordPress.DB.PreparedSQL.NotPrepared`
    *   **Context**: Constructing a dynamic SQL query using `$wpdb->prepare` where the query string itself is built from variables.
    *   **Reason**: The query involves a dynamic list of IDs for an `IN` clause and a variable table name derived from `$wpdb->prefix`. Standard `prepare()` usage requires a literal string, but dynamic construction is necessary here. Placeholders are safely generated, and table names are verified.
